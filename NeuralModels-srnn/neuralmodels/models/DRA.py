...
class DRA(object):
	def __init__(self,edgeRNNs,nodeRNNs,nodeToEdgeConnections,edgeListComplete,cost,nodeLabels,learning_rate,clipnorm=0.0,update_type=RMSprop(),weight_decay=0.0):
		...
		nodeToEdgeConnections is a dictionary with keys as nodeRNNs name and value is another dictionary
		whose keys are edgeRNNs the nodeRNN is connected to and value is
		a list of size-2 which indicates the features to choose from the unConcatenateLayer
		...
		for et in edgeTypes:#connect layers
		...
		for nt in nodeTypes:#torso
			self.params[nt] = []
			self.masterlayer[nt] = unConcatenateVectors(nodeToEdgeConnections[nt])#masterlayer也类似一个字典，key是各个node
			self.X[nt] = self.masterlayer[nt].input

			'''We first connect all the edgeRNNs (building the network bottom-up)'''
			nodeLayers = self.nodeRNNs[nt]#layers of nodeRNN
			edgesConnectedTo = nodeToEdgeConnections[nt].keys()#edges related to the node(nt) nodeToEdgeConnectiions is dictionary, key也是node.
			layers_below = []
			for et in edgeListComplete:
				if et not in edgesConnectedTo:#choose EDGES that connect to the node(nt) in this iteration
					continue
				edgeLayers = self.edgeRNNs[et]
				layers_below.append(edgeLayers)
				edgeLayers[0].input = self.masterlayer[nt].output(et)#edgelayer[0]# call unconcatenateVectors.py
				for l in edgeLayers:
					if hasattr(l,'params'):
			...
			self.Y_pr[nt] = nodeLayers[-1].output()#the last layer ouput of nodeRNN
			self.Y[nt] = self.nodeLabels[nt]#nodeLabels???????
	...
	def fitModel(self,trX,trY,snapshot_rate=1,path=None,epochs=30,batch_size=50,learning_rate=1e-3,
		...
		new_idx=None,featureRange=None,poseDataset=None,graph=None,maxiter=10,unNormalizeData=None):#maxiter改成10
		...
		nodeTypes = self.nodeRNNs.keys()#'torso', 'arm', 'leg'
		...
		'''Converting validation set to a single array when doing drop joint experiments'''
		gth = None
		T1 = -1
		N1 = -1	
		...
		while iterations <= maxiter:
			...
			if not unequalSize:
				...
				for nm in nodeNames: # shuffle data randomly
			...
			for j in range(batches_in_one_epoch):# batches_in_one_epoch = 10
				...
				for nm in nodeNames:
					...
					if(len(tr_X[nt])) == 0:
						...
						tr_X[nt] = np.concatenate((tr_X[nt][:,:,0:trX[nm].shape[2]],trX[nm][:,j*batch_size:min((j+1)*batch_size,numExamples[nm]),:]),axis=1)
						if outputDim == 2:
							tr_Y[nt] = np.concatenate((tr_Y[nt],trY[nm][:,j*batch_size:min((j+1)*batch_size,numExamples[nm])]),axis=1)
						elif outputDim == 3:
							tr_Y[nt] = np.concatenate((tr_Y[nt][:,:,0:trY[nm].shape[2]],trY[nm][:,j*batch_size:min((j+1)*batch_size,numExamples[nm]),:]),axis=1)
`
				loss = 0.0
				skel_loss = 0.0
				grad_norms = []
				...
				'''Trajectory forecasting on validation set'''  '''I CAHNGE IT TO 1'''
			...
			'''if (trX_validation is not None) and (trY_validation is not None) and (not poseDataset.drop_features):
				...
				for nm in trX_validation.keys():
					nt = nm.split(':')[1]
					validation_error += self.predict_node_loss[nt](trX_validation[nm][:,:,0:trY_validation[nm].shape[2]],trY_validation[nm],std)
	...
	def convertToSingleVec(self,X,new_idx,featureRange):
		keys = X.keys()
		[T,N,D]  = X[keys[0]].shape#100 24 10
