...
def readCRFgraph(poseDataset, noise=1e-10, forecast_on_noisy_features=False):
    ...
    nodeFeatureLength = {}
    for node_name, node_type in zip(lines[0].strip().split(','), lines[1].strip().split(',')):#process the first two lines
        nodeOrder.append(node_name)
        nodeNames[node_name] = node_type
        nodeList[node_type] = 0
        nodeToEdgeConnections[node_type] = {}
        nodeToEdgeConnections[node_type][node_type + '_input'] = [0, 0]#three types of input: arm_input, leg_input,torso_input
    ...
    nodeConnections = {}
    edgeListComplete = []
    for i in range(2, len(lines)):#process the last lines of the file
    ...
    for nodeName in nodeNames.keys():## right_arm:arm left_arm: arm
        #print nodeName
        edge_features = {}
        validate_edge_features = {}
        forecast_edge_features = {}

        nodeType = nodeNames[nodeName]#left_leg -> leg; right_arm->arm
        edgeTypesConnectedTo = nodeToEdgeConnections[nodeType].keys()#arm: arm_leg,arm_torso,arm_input; leg: leg_arm,leg_torso,leg_input [ 0 ,0]
        low = 0
        high = 0

        for edgeType in edgeTypesConnectedTo:#arm_arm, torso_arm, arm_input
            print edgeType
            
            [edge_features[edgeType], validate_edge_features[edgeType],
             forecast_edge_features[edgeType]] = poseDataset.getfeatures(nodeName, edgeType, nodeConnections, nodeNames,
                                                                         forecast_on_noisy_features=forecast_on_noisy_features)

        edgeType = nodeType + '_input'
        D = edge_features[edgeType].shape[2]
        nodeFeatureLength[nodeType] = D
        high += D
        nodeToEdgeConnections[nodeType][edgeType][0] = low #nodeToEdgeConnections[nodeType][edgeType]=[ 0 0]intialized before
        ...
        for edgeType in edgeList:#above is about input, this loop is real connections
            ...
            nodeToEdgeConnections[nodeType][edgeType][1] = high
            low = high
            nodeRNNFeatures = np.concatenate((nodeRNNFeatures, edge_features[edgeType]), axis=2)#concatenate edge and node
            validate_nodeRNNFeatures = np.concatenate((validate_nodeRNNFeatures, validate_edge_features[edgeType]),
                                                      axis=2)
            forecast_nodeRNNFeatures = np.concatenate((forecast_nodeRNNFeatures, forecast_edge_features[edgeType]),
                                                      axis=2)

        [Y, Y_validate, Y_forecast, X_forecast, num_classes] = poseDataset.getlabels(nodeName)
        nodeList[nodeType] = num_classes

        idx = nodeName + ':' + nodeType#idx is not number,but right_leg:leg
    ...
    return nodeNames, nodeList, nodeFeatureLength, nodeConnections, edgeList, \
           edgeListComplete, edgeFeatures, nodeToEdgeConnections, \
...
def getNodeFeature(nodeName, nodeFeatures, nodeFeatures_t_1, poseDataset):
    ...
    for edgeType in edgeTypesConnectedTo:
        edge_features[edgeType] = poseDataset.getDRAfeatures(nodeName, edgeType, nodeConnections, nodeNames,
                                                             nodeFeatures, nodeFeatures_t_1)
